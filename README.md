...
File failures contain `error` instead of `extraction`. Request errors contain top-level `error`; malformed JSON has `id: null`. Syntax errors are diagnostic data, not transport errors. Functions with erroneous bodies are omitted. Deleted/defaulted methods without bodies are recorded as omitted. Nested functions/constructs retain preorder. Whole-line ranges can include neighboring text on the same line.

Two bounded signature recoveries require an intact, error-free function body:

- `default_brace_signature_missing_type` accepts the known Tree-sitter default-argument `= {}` missing-type error when every error matches that condition.
- `annotation_prefix_signature` treats a leading return-type token spelled with uppercase ASCII letters, digits or underscores (at least three bytes, including a letter) as a possible object-like annotation macro. It masks only that token with spaces in a temporary copy and reparses the file once. Recovery requires identical body boundaries and function end, unchanged template attachment, and a valid remaining signature (or the default-brace case above). Names, arity and references use the validated tree with original source bytes and conservative original declaration lookup. Returned source ranges include the original annotation.

Uppercase spelling is a syntactic assumption, not proof that a macro is defined or semantically harmless. No preprocessing or macro expansion occurs. Each recovery is reported in `recovered`; `parseHasError` still describes the original parse. Clean files need no extra parse. Function-like annotations, parser damage that loses the function/body structure, and body grammar gaps such as structured binding packs remain unsupported. Some unsupported definitions never form function nodes, so `omitted` is not an exhaustive inventory; callers must also inspect `parseHasError`.
